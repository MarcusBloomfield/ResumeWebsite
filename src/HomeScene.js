    ...
    return (
      <div className="myProjects">
        <Canvas camera={{ position: [-10, 5, 25], fov: 11 }}>
          ...
          <OrbitControls target={this.state.selectedCubePosition} minDistance={44} maxDistance={44} />
          <ambientLight intensity={0.1} />
          <directionalLight color="White" position={[0, 0, 5]} intensity={0.5} castShadow />
          <Suspense fallback={null}>
            <FloatingIsland position={[-3, 0, 0]} rotationAmount={.002} origin={0} />
            <FloatingIsland position={[-3.5, 1, 0]} rotationAmount={-.002} origin={1} />
            <FloatingIsland position={[-3.5, -1, 0]} rotationAmount={-.002} origin={-1} />
...
function FuckCunts(...props) {
  const ref = useRef()

  useFrame((state, delta) => (
    ref.current.target.x > props.cubePosX ? ref.current.target.x -= 1 * delta : ref.current.target.x += 1 * delta,
    ref.current.target.z > props.cubePosZ ? ref.current.target.z -= 1 * delta : ref.current.target.z += 1 * delta,
